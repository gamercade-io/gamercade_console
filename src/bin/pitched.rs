...
use gamercade_audio::{
    get_note, initialize_notes, EnvelopeDefinition, InstrumentId, Phrase, SampleBitDepth,
    SampleDefinition, SamplerInstance,
};
...
fn sampler(output_sample_rate: usize) -> SamplerInstance {
    let reader = WavReader::open("1_piano_mid.wav").unwrap();
...
pub fn main() {
    ...
    let phrase = Phrase::c_scale(InstrumentId(0));
    let mut sample = sampler(output_sample_rate);
    sample.set_frequency(get_note(phrase.entries[0].as_ref().unwrap().note).frequency);
    sample.trigger();
    ...
    std::thread::spawn(move || {
        loop {
            if !buffer_producer.is_full() {
                ...
                if frame_count >= FRAMES_PER_NOTE {
                    index += 1;
                    if let Some(note_id) = phrase.entries[index % 16].as_ref() {
                        sample.set_frequency(get_note(note_id.note).frequency);
                        sample.trigger();
                    } else {
                    }
                    frame_count = 0;
